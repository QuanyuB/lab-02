...
author: "Team Name: Quanyu Bao, Matias Lander, Jason Bi and James Jin and Wayne Wang"
date: "`r Sys.Date()`"
...
plastic_data_all %>% group_by(region) %>% summarise(mean_total = mean(total_mismanage_plastic))

mean_value <- plastic_data_all %>% group_by(region) %>% summarise(mean_total = mean(total_mismanage_plastic))
mean_value %>% arrange(desc(mean_total))
```
...
median_region <- plastic_data_all %>% group_by(region) %>% summarise(median_per_region = median(pct_mismanaged_plastic_ocean)) %>% drop_na()
arrange(median_region, desc(median_per_region))
```
